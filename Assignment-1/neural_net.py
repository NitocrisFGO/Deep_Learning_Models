...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    R_W = reg * np.sum(W1 * W1) + reg * np.sum(W2 * W2)
    ...
    z1 = X.dot(W1) + b1
    a1 = np.maximum(z1, 0)
    da1 = score_to_w.dot(W2.T)
    dz1 = da1 * (z1 > 0)

    db2 = np.sum(score_to_w, axis=0) / X.shape[0]
    dW2 = a1.T.dot(score_to_w) / X.shape[0]
    dW2 += 2 * reg * W2
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in range(num_iters):
      ...
      indexs = np.random.choice(X.shape[0], batch_size)
      X_batch = X[indexs]
      y_batch = y[indexs]
